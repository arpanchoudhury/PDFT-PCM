...
def get_lpdft_hconst(
    ...
    #print('hyb in get_lpdft_hconst', hyb)
...
def make_lpdft_ham_(mc, mo_coeff=None, ci=None, ot=None):
    ...
    """
    print('mc class in make_lpdft_ham_', mc.__class__)
    if mo_coeff is None:
        mo_coeff = mc.mo_coeff
    if ci is None:
        ci = mc.ci
    ...
    #print('ci shape in make_lpdft_ham_', ci[0].shape)
    #print('ci in make_lpdft_ham_ =', ci)
    #print('mo_coeff in make_lpdft_ham_', mo_coeff)
    ot.reset(mol=mc.mol)
    ...
    print('mc.e_mcscf[slice]', mc.e_mcscf[slice(0, len(ci))])
    ...
    h1, h0 = mc.get_h1lpdft(E_ot, casdm1s_0, casdm2_0, hyb=1.0 - cas_hyb, mo_coeff=mo_coeff) # may be this is
    # h1:(1, 2, 3) and h0:(5) number terms in eq 11.
    print('hyb =', hyb)
    h2 = mc.get_h2lpdft()
    ...
    def construct_ham_slice(solver, slice, nelecas):
        ...
        print('lpdft_irrep in construct_ham_slice', lpdft_irrep)
        diag_idx = np.diag_indices_from(lpdft_irrep)
        lpdft_irrep[diag_idx] += h0 + cas_hyb * mc.e_mcscf[slice]

        print('lpdft_irrep in construct_ham_slice', lpdft_irrep)
        print('lpdft_irrep[diag_idx] in construct_ham_slice', lpdft_irrep[diag_idx])
        print('diag_idx in construct_ham_slice', diag_idx)
        return lpdft_irrep
...
def kernel(mc, mo_coeff=None, ci0=None, ot=None, dump_chk=True, **kwargs):
    ...
    #print("mc.si_pdft from original lpdft kernel:\n", mc.si_pdft)
    #print(f"ci_mcscf norm from original lpdft kernel: {np.linalg.norm(ci_mcscf):.6e}")
    #print(f"mc.ci norm from original lpdft kernel: {np.linalg.norm(mc.ci):.6e}")
...
class _LPDFT(mcpdft.MultiStateMCPDFTSolver):
    ...
    def get_lpdft_ham(self):
        ...
        """
        print('self class in get_lpdft_ham', self.__class__)
    ...
    def kernel(self, mo_coeff=None, ci0=None, ot=None, verbose=None, dump_chk=True):
        ...
        print('self class in kernel', self.__class__)
    ...
    def get_lpdft_hcore_only(self, casdm1s_0, hyb=1.0, mo_coeff=None, ncore=None, ncas=None):
        ...
        print('self class in get_lpdft_hcore', self.__class__)
        for cls in type(self).__mro__:
            if 'get_hcore' in cls.__dict__:
                print(cls.__name__, '->', cls.__dict__['get_hcore'])
