...
def energy_tot(mc, mo_coeff=None, ci=None, ot=None, state=0, verbose=None):
    ...
    Args:
        mc : an instance of CASSCF or CASCI class
            ...
            energy. Call mc.kernel () before passing to this function!      ####### THIS IS IMPORTANT #######
    ...
    #print('mo_coeff in energy_tot:', mo_coeff)
    #print('ci vector in energy_tot:', ci)
    #print('ot in energy_tot:', ot)
    # Generate RDMs for the specified state (by default state=0, ground state) 
...
# This function is just for energy computation from the CASSCF wave function 
def energy_mcwfn(mc, mo_coeff=None, ci=None, ot=None, state=0, casdm1s=None,
    ...
    # some density matrix conversions --------
    ...
    Vnn = mc._scf.energy_nuc() # nuclear repulsion energy
...
# Calculates the on-top energy from the precomputed RDMs
...
# Functions for energy decomposition -----------------------------
...
class _PDFT:
    ...
    _mc_class = None  # this gets overwritten by the PDFT() child class in get_mcpdft_child_class()
    ...
    def __init__(self, scf, ncas, nelecas, my_ot=None, grids_level=None,
        ...
        if grids_level is not None:
            ...
            print("grid level in init", grids_level)
    ...
    # (A) Function for the CASSCF OPTIMIZATION ================
    def optimize_mcscf_(self, mo_coeff=None, ci0=None, **kwargs):
        ...
        print("calling real optimize_mcscf_")
    ...
    # (B) Function for the MC-PDFT TOTAL ENERGY calculation (see below energy_tot() function) ==============
    def compute_pdft_energy_(self, mo_coeff=None, ci=None, ot=None, otxc=None,
        ...
        #print('mo_coeff in compute_pdft_energy_:', self.mo_coeff)
        #print('total energy from compute_pdft_energy_:', self.e_tot)
        #print('on-top energy from compute_pdft_energy_:', self.e_ot)
        #print('self.otfinal in compute_pdft_energy_:', self.otfnal)
        #print('self.otxc in compute_pdft_energy_:', self.otxc)
        #print('self.e_ot in compute_pdft_energy_:', self.e_ot)

        epdft = [self.energy_tot(mo_coeff=self.mo_coeff, ci=self.ci, state=ix,
                                 logger_tag='MC-PDFT state {}'.format(ix))
                 for ix in range(nroots)]
        print("epdft list=", epdft)
        self.e_ot = [e_ot for e_tot, e_ot in epdft]
        print("self.e_ot list=", self.e_ot)
        if isinstance(self, StateAverageMCSCFSolver):
            print('yes')
            e_states = [e_tot for e_tot, e_ot in epdft]
            print('self.e_states in compute_pdft_energy_ before try:', getattr(self, 'e_states', None))
            try:
                ...
                print('is here after try?')
                print('self.e_states in compute_pdft_energy_ after try:', self.e_states)
            # TODO: redesign this. MC-SCF e_states is stapled to
            # fcisolver.e_states, but I don't want MS-PDFT to be
            # because that makes no sense
            self.e_tot = np.dot(e_states, self.weights)
            e_states = self.e_states
            print('self.e_states in compute_pdft_energy_:', self.e_states)
        ...
        #print('grids_level in compute_pdft_energy_:', grids_level)
        #print('grids_attr in compute_pdft_energy_:', grids_attr)
        #print("len(grids_attr)", len(grids_attr))
        #print("grids=",self.grids)

        return self.e_tot, self.e_ot, e_states


    # Combined KERNEL for OPTIMIZATION + TOTAL ENERGY calculation ==============
    # Remember: this is the Kernel which gets called whenever pdft.kernel() is called (see at the end of this file)
    def kernel(self, mo_coeff=None, ci0=None, otxc=None, grids_attr=None,
        ...
        #print('Final MC-PDFT energy= ', self.e_tot)
        #print('grids_level in kernel mcpdft.py', grids_level)
        #print('grids_attr in kernel mcpdft.py', grids_attr)
        #print('otxc in kernel mcpdft.py', otxc)
        ...
        #print('mo_coeff in kernel mcpdft.py', self.mo_coeff)
        #print('Final MC-PDFT energy= ', self.e_tot)
    ...
    # Redefine some MC-SCF methods for PDFT (they are in the _dms module) 
    # Was that necessary?
    ...
    def energy_tot(self, mo_coeff=None, ci=None, ot=None, state=0,
        ...
        #print("ot in mcpdft.py", ot)
        #print('mo_coeff in compute_pdft_energy_:', mo_coeff)
        #print('total energy from compute_pdft_energy_:', self.e_tot)
        #print('on-top energy from compute_pdft_energy_:', self.e_ot)
        #print('grids_level in compute_pdft_energy_:', grids_level)
        #print('grids_attr in compute_pdft_energy_:', grids_attr)
...
def get_mcpdft_child_class(mc, ot, **kwargs): # 'mc', 'ot' comes from the user (see __init__.py)
    # Inheritance magic
  
    mc_doc = (mc.__class__.__doc__ or
              'No docstring for MC-SCF parent method')

    # Defines a new class PDFT that inherits from both _PDFT and the class of mc (i.e. mc1step.CASSCF)
    # So the returned object is simultaneously an MC-SCF solver and 
    # an MC-PDFT engine. 
    class PDFT(_PDFT, mc.__class__):   # the order of inheritance matters, _PDFT gets priority when there are name conflicts 
        __doc__ = mc_doc + '\n\n' + _PDFT.__doc__
        _mc_class = mc.__class__  # providing the original MC-SCF class

    # this is where both the MC-PDFT and mc1step.CASSCF initializations are called --------
    ...
    #print('pdft.grids=',pdft.grids)
    return pdft  
    # this return pdft wrapper goes to _MCPDFT.CASSCFMCPDFT 
    # in __init__.py and ready to be run as pdft.kernel() 
