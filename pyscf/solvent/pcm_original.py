...
class PCM(lib.StreamObject):
    ...
    def build(self, ng=None):
        ...
        if self.method.upper() in ['C-PCM', 'CPCM']:
            ...
            #f_epsilon_opt = (epsilon_opt-1.)/epsilon if epsilon_opt != float('inf') else 1.0
            #(epsilon_opt - 1.0)/(epsilon_opt + 1.0)
            f_epsilon_opt = (epsilon_opt-1.0)/(epsilon_opt)# if epsilon_opt != float('inf') else 1.0
            K = S
    ...
    def _get_vind_dyn(self, dms):
        ...
        omeg = K * ((self.refidx)**2 - 1.0)/((self.refidx)**2) 
        additional_ = numpy.dot(omeg, self.q_or[0])
        b = numpy.dot(R_opt, v_grids.T) - additional_
        q = numpy.linalg.solve(K, b).T  # this the charge on the surface
    ...
    def kernel(self, dm, state_id=None, es_method=None, refdm=None):
        ...
        if state_id in [0, None]:
            ...
            _, _, _q_ref, _vgrid_ref = self._get_vind(refdm)    # reference state total charge
            #_q_ref_dyn, _ = self._get_vind_dyn(refdm)           
            #_q_ref_in = _q_ref - _q_ref_dyn 
            _q_ref_in = _q_ref * (self.eps - (self.refidx)**2)/(self.eps - 1.0)
            _q_ref_dyn = _q_ref * ((self.refidx)**2 - 1.0)/(self.eps - 1.0)                      
            self.q_or = _q_ref_in

            _q_dyn, _vgrid = self._get_vind_dyn(dm)             
            #_q_dyn = _q_dyn*((self.refidx)**2 - 1.0)/(self.eps - 1.0)
            ...
            # Calculate polarization free energy according to NEQ model
            grid_coor = self.surface['grid_coords']
            n_grid = grid_coor.shape[0]
            cross_term = 0.0
            for i in range(n_grid):
                for j in range(i + 1, n_grid):
                    grid_dist = numpy.linalg.norm(grid_coor[i]-grid_coor[j])
                    cross_term = cross_term + ( (_q_dyn[0][i] - _q_ref_dyn[0][i]) * _q_ref_in[0][j] )/grid_dist
