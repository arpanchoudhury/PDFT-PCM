...
class PCM(lib.StreamObject):
    ...
    def __init__(self, mol):
        ...
        self.rf_root = None
        # phi_state : int or None
        #   None  → existing SA-NEQ behaviour (rf_root controls fast charges)
        #   k     → Pekar-from-same-SACASSCF: vertical excitation 0 → state k
        #           slow charges from state 0, fast charges from state k,
        #           all within a single SA-CASSCF (no separate refdm needed)
        self.phi_state = None
    ...
    def build(self, ng=None):
        ...
        print('self.radii_table in build', self.radii_table)
        ...
        print("self.method in build", self.method)
        if self.method.upper() in ['C-PCM', 'CPCM', 'COSMO']:
            ...
            print("D shape:", D.shape)        # should be (N, N)
            print("area shape:", area.shape)  # should be (N,)
            print("D * area shape:", (D * area).shape)  # should be (N, N) — broadcasts area along columns
            f_epsilon = (epsilon - 1.0)/(epsilon + 1.0) if epsilon != float('inf') else 1.0
            DA = D * area  # element-wise: D[i,j] * area[j]
            DAS = numpy.dot(DA, S)
    ...
    def _get_vind(self, dms):
        ...
        #print("v_grids_n", self.v_grids_n)
    ...
    def ss_correction(self, dm, refdm=None):
        ...
        if self.equilibrium_solvation == False and self.rf_root is None and self.phi_state is None:
            ...
            if refdm is None:
                _, _, _q0_total, _vgrid0 = self._get_vind(dm[0])
                ...
                #_qfast_sa, _ = self._get_vind_pekar(_sa_dm)   # (1, ngrids)

                slow_self = -0.5 * numpy.dot(_vgrid0[0], _q0_slow[0])
 
                logger.info(self, 'SA-CASSCF NEQ Method II (NEQ[SA:0|SA]):')
                ...
                epcms = []
                for i, d in enumerate(dm):
                    _qfast, _vgrid_i = self._get_vind_pekar(d)
                    v_i = _vgrid_i[0]
 
                    fast_self_k  = 0.5 * numpy.dot(v_i, _qfast[0])   # ½ V^(k)·Q^fast(SA)
                    slow_inter_k = numpy.dot(v_i, _q0_slow[0])           # V^(k)·Q^slow(0)
    ...
    def kernel(self, dm, state_id=None, es_method=None, refdm=None):
        ...
        if state_id in [0, None]:
            ...
            _q0_slow = _q0_total - _q0_fast   # (1, ngrids)
            #
            #print("_q0_slow", _q0_slow.shape)
            #_q0_slow[0]=np.loadtxt('/Users/arpan/solvation/qslow_openmolcas.dat')
            #print("_q0_slow", _q0_slow.shape)
            # ------------------------------------------------------------------
            ...
            print("slow_self =", slow_self)  # = -½ V^(0)·Q^slow
            ...
            print('sum(_q_neq)', numpy.sum(_q_neq))
            #_q_neq  = _q0_slow + (_qphi_fast+_q0_fast)/2 #NOTE may be this is better
            sol_pot = self._get_vmat(_q_neq)[0]
            np.savetxt('qslow_pyscf.dat', _q0_slow[0])
            np.savetxt('qfast_pyscf.dat', _qphi_fast[0])
            # ------------------------------------------------------------------
            ...
            # ==================================================================
            # SA-CASSCF NEQ — Method II
            # NEQ[SA:0|SA]: slow and fast charges both from same SA-CASSCF
            # ------------------------------------------------------------------
            # Pekar partitioning:
            #   K_eps0   Q^(0)      = R_eps0   V^(0)    (total eq, state 0)
            #   K_epsopt Q^fast(0)  = R_epsopt V^(0)    (fast part of state 0)
            #   Q^slow(0) = Q^(0) - Q^fast(0)           (inertial, frozen)
            #   K_epsopt Q^fast(SA) = R_epsopt V^(SA)   (fast from SA density)
            #   Q = Q^fast(SA) + Q^slow(0)              (total NEQ charge)
            #
            # Per-state free energy — identical formula for ALL states k:
            #   G_k = E_k + ½ V^(k)·Q^fast(SA)
            #             + V^(k)·Q^slow(0)
            #             - ½ V^(0)·Q^slow(0)
            #
            # For k=0 this reduces to:
            #   G_0 = E_0 + ½ V^(0)·(Q^fast(SA) + Q^slow(0))
            #   consistent with equilibrium solvation when SA density → state 0
            #
            # Usage:
            #   mc.with_solvent.neq_method = 'II'
            #   mc.with_solvent.refidx     = 1.3328
            # ==================================================================
            _sa_dm    = dm[-1]
            state_dms = dm[:-1]
            ...
            for i, d in enumerate(state_dms):
                ...
                fast_self_k  = 0.5 * numpy.dot(v_i, _qfast_sa[0])   # ½ V^(k)·Q^fast(SA)
                ...
                logger.info(self, '  state %d: ½V^k·Qfast(SA)=%.10g  '
                            'V^k·Qslow=%.10g  -½V^0·Qslow=%.10g  G_P=%.15g',
                            i, fast_self_k, slow_inter_k, slow_self, epcm_i)
            ...
            if partition == 'marcus':
                ...
                #   epcm_k = ½ q_dyn · V^k  +  q_in · V^k  +  drop_term
                epcms = []
                _q_dyn_sum =  np.zeros((1, _q_ref_in.shape[1]))
                for i, d in enumerate(state_dms):
                    if rf_root is not None:
                        _, _, _, _vgrid_k = self._get_vind(dm_fast)
                    else:
        ...
        # compute phi's potential once
            _, _, _, _vgrid_phi = self._get_vind(dm_charges)
            v_phi    = _vgrid_phi[0]
            epcm_phi = 0.5 * numpy.dot(_q[0], v_phi)

            # same correction for all states — only Fock term (sol_pot) is state-dependent
            epcms = [epcm_phi] * len(state_dms)

            for i, e in enumerate(epcms):
                logger.info(self, '  SA-CASSCF EQ G_P state %d = %.15g', i, e)
            '''epcms = []
            for i, d in enumerate(state_dms):
                _, _, _, _vgrid_k = self._get_vind(d)   # always state i's potential
                v_k    = _vgrid_k[0]
                epcm_k = 0.5 * numpy.dot(_q[0], v_k)   # q from rfroot, V from state i
                epcms.append(epcm_k)
