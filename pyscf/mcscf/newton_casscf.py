...
def gen_g_hop(casscf, mo, ci0, eris, verbose=None):
    ...
    print("IN GEN_G_HOP type(casscf)", type(casscf))
    print("IN GEN_G_HOP casscf mro", casscf.__class__.mro())


    diff = casscf.get_hcore() - casscf._scf.get_hcore()

    print(f"In gen_g_hop  hcore norm:        {numpy.linalg.norm(casscf.get_hcore()):.10f}")
    print(f"In gen_g_hop  bare hcore norm:   {numpy.linalg.norm(casscf._scf.get_hcore()):.10f}")
    print(f"In gen_g_hop  diff norm:         {numpy.linalg.norm(diff):.6e}")
    print(f"In gen_g_hop  solvent.v norm:    {numpy.linalg.norm(casscf.with_solvent.v):.6e}" 
          if hasattr(casscf, 'with_solvent') else "  no with_solvent")

    # specific elements to see where difference lives
    print(f"  h[0,0]:      {casscf.get_hcore()[0,0]:.10f}")
    print(f"  h_bare[0,0]: {casscf._scf.get_hcore()[0,0]:.10f}")
    print(f"  diff[0,0]:   {diff[0,0]:.6e}")
