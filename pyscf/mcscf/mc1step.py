...
def kernel(casscf, mo_coeff, tol=1e-7, conv_tol_grad=None,
    ...
    # initial CI guess (from previous calculation?) --------
    if ci0 is None:
        ci0 = casscf.ci
    ...
    # initial CASCI calculation --------
    e_tot, e_cas, fcivec = casscf.casci(mo, ci0, eris, log, locals())
    #print('1st CASCI')
# macro iterations are needed when added solvent model
#    print('check if ncas == nmo and not casscf.internal_rotation:', ncas == nmo, casscf.internal_rotation)
...
#        return True, e_tot, e_cas, fcivec, mo, mo_energy
    ...
    while not conv and imacro < casscf.max_cycle_macro:
        ...
        # PySCF adjusts stepsize and number of micro-iterations dynamically!!! --------
        ...
        for u, g_orb, njk, r0 in rota:
            ...
            # update CASCI wavefunction using the rotated orbitals --------
            #print('before update_casdm in mc1step')
            casdm1, casdm2, gci, fcivec = \
                    casscf.update_casdm(mo, u, fcivec, e_cas, eris, locals())
            #print('gci in kernel', gci)
            #print('after update_casdm in mc1step')
            ...
            if (norm_t < conv_tol_grad or
                ...
                break
            #print('before next micro iteration in mc1step')
        # -------- End of micro iterations -------
        ...
        # After micro iteration, apply final orbital rotation --------
        mo = casscf.rotate_mo(mo, u, log)

        # Update integrals after orbital rotation --------
        eris = casscf.ao2mo(mo)

        
        t2m = log.timer('update eri', *t3m)

        # Check whether the orbital rotation is “small” --------
        max_offdiag_u = numpy.abs(numpy.triu(u, 1)).max()

        # If the orbital rotation is small enough, the expensive CI-vector update 
        # can be skipped --------
        ...
        # Reuse the previous CI vector when the orbital rotation is small 
        # BUT the small_rot criterion is only used for custom FCI solvers,
        # not for builtin FCI solvers  --------

        # CATCH: The logic also differs depending on whether the CASSCF is 
        # state-averaged or state-specific
        if not isinstance(casscf, StateAverageMCSCFSolver): # In state-specific CASSCF
            
            # The fcivec from builtin FCI solver is a numpy.ndarray, but if it is not,
            # then a custom FCI solver is used, and can be skipped when the rotation is small
            if not isinstance(fcivec, numpy.ndarray):
                fcivec = small_rot

        else: # In state-averaged CASSCF
            newvecs = []
            for subvec in fcivec:
                # CI vector obtained by builtin FCI is a numpy array 
                # (and same logic as above)
        ...
        # Run CASCI calculation with new orbitals --------
        e_tot, e_cas, fcivec = casscf.casci(mo, fcivec, eris, log, locals())
        #print('2nd CASCI')
        #log.info('fcivec shape: %s', str(fcivec.shape) if isinstance(fcivec, numpy.ndarray) else str([subvec.shape for subvec in fcivec]))

        # Compute new density matrices --------
        #print('before make1rdm in mc1step')
        casdm1, casdm2 = casscf.fcisolver.make_rdm12(fcivec, ncas, casscf.nelecas)
        #print('after make1rdm in mc1step')

        #print("mo_coeff from mc1step:", mo)
        #print("ci_vector from mc1step:", fcivec)
        #numpy.save('mo_coeff_mc1step.npy', mo)
        #numpy.save('ci_vector_mc1step.npy', fcivec) 
        ...
        # Macro iteration convergence check --------
        ...
        #print('before dump chk in mc1step')
        if dump_chk and casscf.chkfile:
            casscf.dump_chk(locals())
        #print('after dump chk in mc1step')

        #print('before callback in mc1step')
        if callable(callback):
            callback(locals())
        #print('after callback in mc1step')

    if conv:
        log.info('1-step CASSCF converged in %3d macro (%3d JK %3d micro) steps',
                 imacro, totinner, totmicro)
        #print('dm from mc1step after conv=true:', casscf.make_rdm1())
    else:
        log.info('1-step CASSCF not converged, %3d macro (%3d JK %3d micro) steps',
                 imacro, totinner, totmicro)


    #print("abs(de),norm_gorb0, norm_ddm, max_offdiag_u,  casscf.small_rot_tol, norm_gci", 
    #      abs(de), norm_gorb0, norm_ddm, max_offdiag_u, casscf.small_rot_tol, norm_gci)
    # If final canonicalization and natural orbitals are requested
    if casscf.canonicalization:
    ...
    #numpy.save('mo_cano_coeff_mc1step.npy', mo)
    #numpy.save('ci_cano_vector_mc1step.npy', fcivec) 
    
    #print('dm from mc1step after canonicalization:', casscf.make_rdm1())
    if dump_chk and casscf.chkfile:
        casscf.dump_chk(locals())

    log.timer('1-step CASSCF', *cput0)

    #print('mo_coeff from mc1step after canonicalization:', mo)
    #print('ci_vector from mc1step after canonicalization:', fcivec)
...
''',
    ...
    def kernel(self, mo_coeff=None, ci0=None, callback=None, _kern=kernel):
        ...
        self._finalize()
        #print('mo_coeff after casscf kernel:', self.mo_coeff)
    ...
    def casci(self, mo_coeff, ci0=None, eris=None, verbose=None, envs=None):
        #print('in top of casci in mc1step')
        log = logger.new_logger(self, verbose)
        fcasci = _fake_h_for_fast_casci(self, mo_coeff, eris)
        #print('-->')
        e_tot, e_cas, fcivec = casci.kernel(fcasci, mo_coeff, ci0, log,
                                            envs=envs)
        #print('fcivec size:', fcivec.shape if isinstance(fcivec, numpy.ndarray) else [subvec.shape for subvec in fcivec])
        #print('-->')
        #print('after casci.kernel in mc1step')
        #print('fcivec from casci:', fcivec.shape if isinstance(fcivec, numpy.ndarray) else str([subvec.shape for subvec in fcivec]))
        ...
        #print('in bottom of casci in mc1step')
