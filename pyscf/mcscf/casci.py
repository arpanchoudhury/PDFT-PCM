...
def analyze(casscf, mo_coeff=None, ci=None, verbose=None,
    ...
    print("ci in casci analyze:", civec)
    print("ci[0]=",civec[0])

    if getattr(casscf.fcisolver, 'make_rdm1s', None):
        print('if casscf.fcisolver', getattr(casscf.fcisolver, 'make_rdm1s', None))
        ...
        dm1 = dm1a + dm1b
        print('dm1_avg.shape', dm1.shape if isinstance(dm1, numpy.ndarray) else len(dm1))
        print("dm1_avg=", dm1)
        spin_dm1 = dm1a - dm1b
...
def kernel(casci, mo_coeff=None, ci0=None, verbose=logger.NOTE, envs=None):
    ...
    print('fcivec.shape', fcivec.shape if isinstance(fcivec, numpy.ndarray) else len(fcivec))
...
# don't call self._scf.get_veff because _scf might be DFT object
    ...
    def make_rdm1(self, mo_coeff=None, ci=None, ncas=None, nelecas=None,
        ...
        #print("casdm1 in make_rdm1 of CASBase class in casci.py is ", casdm1)
        #print("put casdm here in make_rdm1 of CASBase class in casci.py")
        mocore = mo_coeff[:,:ncore]
        mocas = mo_coeff[:,ncore:ncore+ncas]
        dm1 = numpy.dot(mocore, mocore.conj().T) * 2
        dm1 = dm1 + reduce(numpy.dot, (mocas, casdm1, mocas.conj().T))
        #print("sa-casdm in make_rdm1 of CASBase class in casci.py is ", casdm1)
        #print("dm1 in CASBase class in casci.py is ", dm1)
        #print("mocoeff in make_rdm1 of CASBase class in casci.py is ", mo_coeff)
        return dm1
