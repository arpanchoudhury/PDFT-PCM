...
class StateAverageFCISolver:
    ...
    def kernel(self, h1, h2, norb, nelec, ci0=None, **kwargs):
        ...
        #print('before super kernel in addons')
        fcibase = super()
        ...
        self.e_states = e
        #print('after super kernel in addons')
        #print("self.e_states from og", self.e_states)
        #print("fcivec from og", c)
        ...
        #print("self.e_states from addons.py", self.e_states)
        #print("state-averaged energy from addons.py",numpy.einsum('i,i->', e, self.weights))
    ...
    def states_make_rdm1(self, ci0, norb, nelec, *args, **kwargs):
        ...
        #log = logger.new_logger(self, kwargs.get('verbose'))
        #log.note(('type(self) = %s'), str(type(self)))
        #log.note('dm1 shape = %s', str(dm1.shape) if isinstance(dm1, numpy.ndarray) else str([subvec.shape for subvec in dm1]))
        #log.note('Now accessing the desired state-averaged all states 1-particle density matrix')
        #print("states_make_rdm1 from addons.py is being called. DM=", dm1)
        #print("ci0 from states_make_rdm1", ci0)
        #print("states_make_rdm1 shape", [dm.shape for dm in dm1])
        return dm1

    def _sa_solvation(self, ci0, norb, nelec, *args, **kwargs):
        #print("ci0 from _sa_solvation", ci0)

        fcibase = super()
        _dm1 = [fcibase.make_rdm1(c, norb, nelec, *args, **kwargs) for c in ci0]
        return self.e_states, self.weights, _dm1
    
    def make_rdm1(self, ci0, norb, nelec, *args, **kwargs):
        #log = logger.new_logger(self, kwargs.get('verbose'))
        #print("GREAT! make_rdm1 from addons.py is being called. This is the SA-dm")
        #print("individual states dm", self.states_make_rdm1(ci0, norb, nelec, *args, **kwargs))
        #print("SA-dm from addons.py", sum ([w * dm for w, dm in zip(self.weights, #self.states_make_rdm1(ci0, norb, nelec, *args, **kwargs))]))
        #print("SA-dm.shape", sum ([w * dm for w, dm in zip(self.weights,self.states_make_rdm1(ci0, norb, nelec, *args, **kwargs))]).shape)
